...
export const CetakPengadaan = () => {
  ...
  useEffect(()=>{
    ...
    const iptData = [
      {
        pengesahan : "Tanggal",
        diajukan : formatInTimeZone(new Date( location.state.tanggal ), 'Asia/Jakarta', 'dd/MM/yyyy', { locale: id }),
        disetujui : formatInTimeZone(new Date( listData[0].tgl_verify ), 'Asia/Jakarta', 'dd/MM/yyyy', { locale: id }),
        register : formatInTimeZone(new Date( listData[0].tgl_verify ), 'Asia/Jakarta', 'dd/MM/yyyy', { locale: id }),
      },
      {
        pengesahan : "Nama",
        diajukan : listData[0].user[0].pemohon,
        disetujui : "Rusli Adna",
        register : "Endang Wahyu W" 
      },
      {
        pengesahan : "Jabatan",
        diajukan : listData[0].user[0].jabatan,
        disetujui : "Plan Manager",
        register : "Supervisor" 
      },
      {
        pengesahan : "Departemen",
        diajukan : listData[0].user[0].divisi,
        disetujui : "",
        register : "Purchasing" 
      },
    ]
    setList(iptData)
    console.log(listData[0].user[0].pemohon)
    setImg([
      {
                ...
                <View style={styles.table}>
                    ...
                    {dataPengadaan.map((row, i) => {
                      ...
                      for(let x = 0;x < row.parsial_data.length ; x++){
                        let xDate = new Date(row.parsial_data[x].tglDatang);
                        let newPilih = xDate.toLocaleDateString("id-ID", {day: '2-digit', month: 'long', year: 'numeric'});
                        let terima = `${newPilih} (${row.parsial_data[x].qty}),`
                        t_terima += terima
                      }
                      //
                      return(
                        <View key={i} style={styles.row} wrap={true}>
                          ...
                          <View style={[styles.row2, {borderBottom:'none'}]}>
                            <Text style={{padding: 2, textAlign: 'left'}}>{row.spesifikasi}</Text>
                          </View>
