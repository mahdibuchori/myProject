...
export const UpdateOrder = () => {
    ...
    const cekData =async () =>{
        setIsLoading(true);
        await partFalse()
        await onPart(location.state.data.id_pengajuan,userData.user_plan);
    }

    const cekDataUpdate = async () =>{
        setStatusItem("PENGAJUAN");
        setKode(location.state.data.id_pengajuan);
        setOrderQty(`${dataPart.qty_order}`);
        setKeterangan(`${dataPart.keterangan}`)
        setIsLoading(false)
    }
    ...
    const updateData = async () =>{
        if(userData.user_name === dataPart.pemohon){
            try {
                console.log({
                ...
                const next = await API_AUTH.put(`/orderPartUpdate`, {
                    "id_item" : location.state.data.id_item,
                    "id_order" : kode,
                    "nama_item" : location.state.data.nama_item,
                    "qty_order" : orderQty,
            ...
            Swal.fire('info','Tidak dapat melakukan proses update data karena username dan pemohon tidak sama','warning');
        }
    }
