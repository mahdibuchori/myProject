...
export const ChartKimia = (props) => {
  ...
  const onGridReady = (x) =>{
      ...
      const kimia = dataKimia.filter((v,i,a)=>a.findIndex(v2=>(v2.item===v.item))===i);
      let material = kimia.map(d => { 
      ...
      if(material.length > 0){
          let nama = dataKimia[0].value;
          const postIds = dataKimia.map((e) => {
              return(
