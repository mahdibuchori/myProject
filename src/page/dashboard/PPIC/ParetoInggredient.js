...
export const ParetoInggredient = (props) => {
    ...
    const onGridReady = () =>{
        ...
        if(year > 2022 && year <2024){
            const data = dataInggredient.filter(x => x.bulan === buls[month]);
    ...
    const performRefresh = () =>{
        if(props.name === ""){
            ...
            try {
                ...
                const dataInggredient = dataDashboard.inggredient;
                if(year > 2022 && year <2024){
                    const data = dataInggredient.filter(x => x.bulan === buls[bln]);
                    const postIds = data.map((e) => {
                        const ttl = parseFloat(e.total).toFixed(2);
                        const newTotal = (parseFloat(ttl) * 1000 ) / 1000;
                        return(
    ...
    return (
        <>
            <h6 className=''>Top 10 Monthly Inggredient</h6>
            <div>
                <ListGroup numbered>
                    {fileName.map((e, i) =>{
                        return(
